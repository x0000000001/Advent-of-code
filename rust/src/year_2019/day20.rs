...
type Portals = HashMap<usize, Vec<((usize, usize), bool)>>;
                ...
                State::Portal(hash) => {
                    if (hash == aa_hash || hash == zz_hash) && depth != 0 {
                        ...
                        let is_outer = {
                            let index = exits
                                .iter()
                                .position(|((px, py), _)| cx.abs_diff(*px) + cy.abs_diff(*py) < 2)
                                .unwrap();
                            let ((_, _), is_outer) = exits.remove(index);
                            is_outer
                        };

                        exits = exits
                            .into_iter()
                            .filter(|((px, py), _)| cx.abs_diff(*px) + cy.abs_diff(*py) >= 2)
                            .collect();
                        // END REPLACED WITH
