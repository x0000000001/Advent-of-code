...
type InputType = (Vec<Vec<Vec<char>>>, Vec<Vec<Vec<char>>>);
...
pub fn part1(s: String) -> Solution {
    ...
    for list in codes {
        count += list
            .iter()
            .filter(|el| possible_unique_vals.contains(&el.len()))
            .count();
    }

    Solution::from(count as i32)
}
...
fn decode(patterns: &Vec<Vec<char>>, codes: &Vec<Vec<char>>) -> i32 {
    ...
    last_char_linked = *seven_pattern
        .iter()
        .filter(|el| !one_pattern.contains(el))
        .next()
        .unwrap();
    linking.insert('a', last_char_linked);
    ...
    linking.insert('b', last_char_linked);
    to_link.retain(|x| *x != last_char_linked);
    ...
    linking.insert('e', last_char_linked);
    to_link.retain(|x| *x != last_char_linked);
    ...
    linking.insert('f', last_char_linked);
    to_link.retain(|x| *x != last_char_linked);
    ...
    last_char_linked = *apparitions
        ...
        .unwrap()
        .0;
    linking.insert('c', last_char_linked);
    to_link.retain(|x| *x != last_char_linked);
    ...
    for c in to_link.iter() {
        if four_pattern.contains(&c) {
            last_char_linked = *c;
            linking.insert('d', last_char_linked);
            break;
        }
    }
    to_link.retain(|x| *x != last_char_linked);
    ...
    linking.insert('g', last_char_linked);
    to_link.retain(|x| *x != last_char_linked);
...
fn parse(s: String) -> InputType {
    ...
    for line in input {
        let words: Vec<Vec<char>> = line
            .split_whitespace()
            .map(|el| el.chars().into_iter().collect())
            .collect();
