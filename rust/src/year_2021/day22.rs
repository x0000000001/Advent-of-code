...
type InputType = Vec<(bool, Cuboïd)>;
...
impl Cuboïd {
    ...
    fn max_amplitude(&self) -> i64 {
        return self.x0.abs().max(
            self.x1.abs().max(
                self.y0
                    .abs()
                    .max(self.y1.abs().max(self.z0.abs().max(self.z1.abs()))),
            ),
        );
    }
}
...
pub fn part1(s: String) -> Solution {
    ...
    let acceptable_moves: Vec<(bool, Cuboïd)> = input
        .into_iter()
        .filter(|c| c.1.max_amplitude() <= 50)
        .collect();

    Solution::from(count_blocks(&acceptable_moves))
}

pub fn part2(s: String) -> Solution {
    let input = parse(s);

    Solution::from(count_blocks(&input))
}
