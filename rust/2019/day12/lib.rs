...
pub fn result_1(mut pos: InputType) -> i64 {
    ...
    (0..pos.len()).map(|i| pos[i].sum() * speeds[i].sum()).sum()
}
...
pub fn result_2(input: InputType) -> i64 {
    let mut xpos = input.iter().map(|p| p.x).collect::<Vec<i64>>();
    let mut ypos = input.iter().map(|p| p.y).collect::<Vec<i64>>();
    let mut zpos = input.iter().map(|p| p.z).collect::<Vec<i64>>();
    ...
    num_integer::lcm(iz as i64, num_integer::lcm(ix as i64, iy as i64))
}
...
pub fn read_input(path: &str) -> InputType {
    ...
    input.iter().map(|l| read_vec(l)).collect()
}
