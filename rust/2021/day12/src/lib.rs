...
{  
    ...
    let mut links: Vec<(String, String)> = input.iter().map(
        |l| {
...
fn get_paths2(current_point: String ,links: Vec<(String, String)>, previous_path: Vec<String>, small_cave_twice: bool) -> Vec<Vec<String>>{
    ...
    for link in links.iter() {
        ...
        if let Some(next_val) = next {
            ...
            let new_small_cave_twice = small_cave_twice || (previous_path.contains(&current_point) && current_is_lower);

            let next_is_lower: bool = next_val.to_uppercase() != next_val;
            
            if new_small_cave_twice && next_is_lower && new_path.contains(&String::from(next_val)) {continue;}
            ...
            let new_potential_ends = get_paths2(String::from(next_val), new_available_links, new_path, new_small_cave_twice);

            for e in new_potential_ends {
                if e.last().unwrap() != "end" {continue;} 
