...
fn knot_hash(s: &str) -> Vec<char> {
    let mut ascci_codes: Vec<usize> = s.chars().map(|c| {
        let mut buffer = [0;1];
...
{   
    (0..128).flat_map(|i| string_to_bits(&knot_hash(&format!("{}-{}",input,i)))).filter(|b| *b).count() as i64
}


pub fn result_2(input: InputType) -> i64
{   
    let grid = (0..128).map(|i| string_to_bits(&knot_hash(&format!("{}-{}",input,i)))).collect::<Vec<Vec<bool>>>();
...
mod test 
{
    ...
    fn test1()
    {
        assert_eq!(result_1("flqrgnkx".to_owned()), 8108);
    }
    ...
    fn test2()
    {
        assert_eq!(result_2("flqrgnkx".to_owned()), 1242);
    }
}