...
pub fn next(s: &mut String, index: usize) {
    ...
    let next_char = ASCII_LOWER[(ASCII_LOWER.iter().position(|el| *el == last_char).unwrap() +1)%26];
...
pub fn result_1(input: InputType) -> String
{
    next_password(input)
}


pub fn result_2(mut input: InputType) -> String
{   
