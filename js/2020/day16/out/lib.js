...
const f1 = ([fields, personnal_ticket, tickets]) => {
    ...
    let possibilities = Array.from({
        ...
        original_index,
        new Set(Array.from({ length: fields.length })
            ...
            for (let ticket_index = 0; ticket_index < tickets.length; ticket_index++) {
                ...
                if (!follows_rule(tickets[ticket_index][candidate_index], fields[original_index])) {
                    return null;
                }
            }
    ...
    return possibilities
        ...
        const used_index = [...possibilities].filter((x) => !used.has(x))[0];
