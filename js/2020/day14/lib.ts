...
const number_to_bits36 = (x: number): boolean[] => {
...
const bits36_to_number = (bits: boolean[]): number => {
  let x = 0;
...
const apply_mask = (bits: boolean[], mask: number[]): boolean[] => {
  return new Array(36)
    .fill(false)
...
const f0 = (input) => {
  ...
  for (let i = 0; i < input.length; i++) {
    ...
    switch (command) {
      ...
      case "mem":
        mem[input[i][1]] = apply_mask(
          number_to_bits36(input[i][2]),
          current_mask
        );
        break;
    }
...
const next_combination = (l: boolean[], index = 0): boolean => {
  if (index === l.length || index === -1) return false;
...
const apply_mask1 = (bits: boolean[], mask: number[]): boolean[] => {
  return new Array(36)
    .fill(false)
    .map((_, i) => (mask[i] === 1 ? true : bits[i]));
};

const possible_addresses = (address: number, mask: number[]): number[] => {
  let possibilities = [];
  ...
  let floating_bits_addresses = mask
    ...
    .filter(([x, _]) => x === 2)
    .map(([_, i]) => i);
  let current_combination = new Array(floating_bits_addresses.length).fill(
    false
  );
