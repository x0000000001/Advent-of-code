...
fn basin_size(min_location: (i32, i32), heightmap: &Vec<Vec<u8>>) -> i32 {
    ...
    while to_explore_cases.len() != 0 {
        ...
        for i in 0..initial_length {
            ...
            if x != 0
                && heightmap[(x - 1) as usize][y as usize] != 9
                && heightmap[(x - 1) as usize][y as usize] > heightmap[x as usize][y as usize]
                && !explored_cases.contains(&(x - 1, y))
                && !to_explore_cases.contains(&(x - 1, y))
            {
                to_explore_cases.push((x - 1, y));
            }
            if y != 0
                && heightmap[x as usize][(y - 1) as usize] != 9
                && heightmap[x as usize][(y - 1) as usize] > heightmap[x as usize][y as usize]
                && !explored_cases.contains(&(x, y - 1))
                && !to_explore_cases.contains(&(x, y - 1))
            {
...
pub fn part2(s: String) -> Solution {
    ...
    let mut basins_sizes: Vec<i32> = min_locations
        .iter()
        .map(|el| basin_size(*el, &heightmap))
        .collect();
    basins_sizes.sort();
    basins_sizes.reverse();

    Solution::from(basins_sizes[0] * basins_sizes[1] * basins_sizes[2])
}
