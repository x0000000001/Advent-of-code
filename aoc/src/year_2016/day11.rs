...
type InputType = (Vec<Vec<(bool, u64)>>, usize);
...
fn get_possible_paths((elev, floor): &InputType) -> Vec<InputType> {
    ...
    if !are_bottom_floors_empty && floor > 0 {
        adjacent_floors.push(floor - 1);
    }
...
pub fn part1(s: String) -> Solution {
    ...
    Solution::from(djisktra_elevator(input))
}
    ...
    for l in input {
        ...
        while i < w.len() {
            ...
            let name = if is_microchip {
                w[i].split("-").collect::<Vec<&str>>()[0].to_string()
            } else {
                w[i].to_string()
            };
